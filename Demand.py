...
df = pd.read_fwf('../03_SDDP_DATABASE/02_DEMAND/dbf005bo.dat', encoding='cp1252', header=0, colspecs="infer", engine='python')
...
filtered_demand = filtered_demand.copy()
...
dfc = pd.read_fwf('../03_SDDP_DATABASE/03_NTC/dcirc.dat',header=1,colspecs="infer", engine='python', encoding='cp1252')
...
buslist = pd.read_csv('../03_SDDP_DATABASE/01_POWER_PLANTS/BusList-ZONES1.csv', encoding='cp1252')
zones = dict(zip(buslist['Busname'], buslist['Zones']))
finaldemand = finaldemand.groupby(zones, axis=1).sum()

# finaldemand.to_csv('../04_DISPASET_DATABASE/Demand/Demand.csv')